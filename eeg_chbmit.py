...
def train(trainloader, model, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in tqdm(enumerate(trainloader)):
       ...
       # print("------------")
        ...
        y_dim = np.shape(te_Y_onehot)[1]

        if y_dim == 1:
            y_type = 'continuous'  # 连续的
        elif y_dim == 2:
            y_type = 'binary'  # 二进制的
        else:
            y_type = 'categorical'  # 分类的
        ...
        model1 = DeepIMV_AISTATS(sess, "DeepIMV_AISTATS", input_dims, network_settings)  # 在这里调用IB
