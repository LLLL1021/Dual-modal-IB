...
### DEFINE STOCHASTIC ENCODER 随机编码器
...
### DEFINE SUPERVISED LOSS FUNCTION 损失函数
...
### DEFINE NETWORK-RELATED FUNCTIONS 定义与网络相关的功能
...
###########################################################################
#### DEFINE PROPOSED-NETWORK 定义整个IB网络
class DeepIMV_AISTATS:
    '''
        ...
        -添加混合模式
        -删除公共/共享部分--返回到以前的版本
        -保持一致性损失；但一定要将gamma设置为0
    '''

    #input_dims, network_settings都输main里面的函数
    def __init__(self, sess, name, input_dims, network_settings):
        ...
        self.z_dim            = input_dims['z_dim']  # z_dim is equivalent to W and Z  z_dim相当于W和Z
        self.steps_per_batch  = input_dims['steps_per_batch']
        
        # PREDICTOR INFO (VIEW-SPECIFC) #单视图的预测器
        ...
        # PREDICTOR INFO (MULTI_VIEW)#双视图的预测器
...
#         with tf.name_scope(self.name):
        with tf.variable_scope(self.name):
            ...
            ### PRIOR   #先验？
            prior_z  = ds.Normal(0.0, 1.0) #PoE Prior - q(z)  #normal()正态分布随机数的生成函数是normal()
            prior_z_set = {}
            for m in range(self.M):
